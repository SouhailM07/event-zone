<x-templates.home-template :title="__('events.home')" class="overflow-x-hidden">
    <h1 class="text-3xl text-gray-900 mb-6 flexBetween">
        <p>{{ __('events.results_for') }}: <span class="font-bold">{{ $title }}</span></p>
        <span>{{ $totalEvents }}</span>
    </h1>

    @forelse($eventsByCategory as $category)
        <h2 class="text-2xl font-bold text-gray-900 my-6">{{ __('events.type_' . $category['type']) }}</h2>
        ...
        <div class="swiper mySwiper">
            <div class="swiper-wrapper gap-[1rem] min-h-[20rem]">
                ...
                @empty
                    <p class="text-gray-500 text-center w-full py-4">
